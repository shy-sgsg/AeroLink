...
struct AuxHeader {
    // 雷达参数
    int64_t op_mode;    // 工作模式（0:单通道, 1:全极化, 2:单极化双天线干涉, 3:单极化多天线干涉, 4:全极化双天线干涉）
    int64_t pp_mode;    // 乒乓模式（0:标准模式, 1:乒乓模式）
    int64_t Kr_sign;    // 调频率符号（0:负, 1:正）
    double fc;          // 载频（Hz）
    ...
    // 图像参数
    int64_t pulse_num;  // 行数（图像高度，方位点数）
    int64_t pulse_len;  // 列数（图像宽度，距离点数）
    int64_t amp_bit;    // 幅度图像数据位数（bit）
    double Xbin;        // 行间间距（方位向像素宽度）（m）
    double Rbin;        // 列间间距（距离向像素宽度）（m）

    // 几何参数
    int64_t geo_mode;   // 成像几何模式（0:偏移固定 fdc, 1:偏移变化 fdc, 2:正侧视）
    int64_t look_mode;  // 侧视模式（0:右侧视, 1:左侧视）
    int64_t flag_flat;  // 是否去平地（0:未去平地, 1:已去平地）
    ...
    int64_t az_MLK_num; // 未在文档中说明，可能为预留字段
};
